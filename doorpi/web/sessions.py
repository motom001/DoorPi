...
class SessionHandler:
    ...
    def build_security_object(
        ...
        web_session["groups"] = {
            group
            for group, users in conf.view("groups").items()
            if username in users
        }
        read_areas = {
            area
            for area, groups in conf.view("readaccess").items()
            if web_session["groups"] & set(groups)
        }
        write_areas = {
            area
            for area, groups in conf.view("writeaccess").items()
            if web_session["groups"] & set(groups)
        }
