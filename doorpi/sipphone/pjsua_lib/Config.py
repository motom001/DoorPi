...
def pj_log(level, msg, length):
    ...
    if level == 4:
        logger.trace("PJ: %s", msg)
    if level == 3:
        logger.debug("PJ: %s", msg)
    if level == 2:
        logger.info("PJ: %s", msg)
    if level == 1:
        logger.warning("PJ: %s", msg)
    if level == 0:
        logger.error("PJ: %s", msg)
...
def create_MediaConfig():
    ...
    MediaConfig.audio_frame_ptime = conf.get_int(
        SIPPHONE_SECTION, 'media.audio_frame_ptime', 20)
    MediaConfig.channel_count = conf.get_int(
        SIPPHONE_SECTION, 'media.channel_count', 1)
    MediaConfig.clock_rate = conf.get_int(
        SIPPHONE_SECTION, 'media.clock_rate', 8000)
    MediaConfig.ec_options = conf.get_int(
        SIPPHONE_SECTION, 'media.ec_options', 0)
    MediaConfig.ec_tail_len = conf.get_int(
        SIPPHONE_SECTION, 'media.ec_tail_len', 1024)
    MediaConfig.enable_ice = conf.get_bool(
        SIPPHONE_SECTION, 'media.enable_ice', True)
    MediaConfig.enable_turn = conf.get_bool(
        SIPPHONE_SECTION, 'media.enable_turn', False)
    MediaConfig.ilbc_mode = conf.get_int(
        SIPPHONE_SECTION, 'media.ilbc_mode', 30)
    ...
    MediaConfig.max_media_ports = conf.get_int(
        SIPPHONE_SECTION, 'media.max_media_ports', 32)
    ...
    MediaConfig.rx_drop_pct = conf.get_int(
        SIPPHONE_SECTION, 'media.rx_drop_pct', 0)
    MediaConfig.snd_auto_close_time = conf.get_int(
        SIPPHONE_SECTION, 'media.snd_auto_close_time', 5)
    MediaConfig.snd_clock_rate = conf.get_int(
        SIPPHONE_SECTION, 'media.snd_clock_rate', 0)
    MediaConfig.turn_conn_type = conf.get_int(
        SIPPHONE_SECTION, 'media.turn_conn_type', 17)
    # TODO: string to http://www.pjsip.org/python/pjsua.htm#AuthCred
    MediaConfig.turn_cred = None
    MediaConfig.turn_server = conf.get(
        SIPPHONE_SECTION, 'media.turn_server', '')
    MediaConfig.tx_drop_pct = conf.get_int(
        SIPPHONE_SECTION, 'media.tx_drop_pct', 0)
    return MediaConfig
...
def create_AccountConfig():
    ...
    AccountConfig.reg_timeout = conf.get_int(
        SIPPHONE_SECTION, 'account.reg_timeout', 10)
