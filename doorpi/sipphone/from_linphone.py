...
class LinPhone(SipphoneAbstractBaseClass):
    ...
    def start(self):
        ...
        self.core.sip_transports = lin.SipTransports(conf.get_int(SIPPHONE_SECTION, 'local_port', 5060),
    ...
    def self_check(self, *args, **kwargs):
        ...
        if self.current_call.state < lin.CallState.Connected:
            if self.current_call_duration > self.core.inc_timeout - 0.5:
                logger.info("call timeout - hangup current call after %s seconds (max. %s)", self.current_call_duration,
                            self.core.inc_timeout)
                self.core.terminate_all_calls()
