...
def create_daemon_file(daemon_file_template, daemon_file_target):
    logger.info("start to create daemon file now...")
    ...
    logger.info("finished writing - now set mod to 0755")
    os.chmod(daemon_file_target, 0777)
    logger.info("finished create daemon file")
    return True
...
def entry_point():
    ...
    parser.add_argument("--logfile_level", default=CONST.LOG_LEVEL, action="store", dest="logfile_level",
                        choices=possible_log_levels, help="Set the logging level for the logfile")
    ...
    if args.daemon.lower() == "stop":
        ...
        logger.info("send SIGTERM to DoorPi with pi %s" % daemon_pid)
    ...
    try:
        ...
        logger.exception("CorruptConfigFileException: %s", ex)
