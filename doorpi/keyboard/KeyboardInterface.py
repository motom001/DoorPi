...
def load_keyboard():
    ...
    if len(config_keyboards) > 0:
        logger.info("using multi-keyboard mode (keyboards: %s)",
                    ', '.join(config_keyboards))
...
def load_single_keyboard(keyboard_name):
    ...
    keyboard_type = doorpi.DoorPi().config.get(
        'keyboards', keyboard_name, 'dummy').lower()
...
class KeyboardHandler(KeyboardAbstractBaseClass):
    ...
    def output_status(self):
        ...
        for Keyboard in self.__keyboards:
            for pin in self.__keyboards[Keyboard].output_pins:
                return_dict[Keyboard+'.' +
                            str(pin)] = self.__keyboards[Keyboard].status_output(pin)
        return return_dict
    ...
    def __init__(self, config_keyboards):
        ...
        for keyboard_name in config_keyboards:
            logger.info("trying to add keyboard '%s' to handler",
                        keyboard_name)
            self.__keyboards[keyboard_name] = load_single_keyboard(
                keyboard_name)
            ...
            for output_pin in output_pins:
                output_pin_name = doorpi.DoorPi().config.get(
                    keyboard_name+'_OutputPins', output_pin)
    ...
    def set_output(self, pin, value, log_output=True):
        if pin not in self.__OutputMappingTable:
            raise UnknownOutputPin('outputpin with name %s is unknown %s' % (
                pin, self.__OutputMappingTable))
        self.__keyboards[self.__OutputMappingTable[pin]
                         ].set_output(pin, value, log_output)
