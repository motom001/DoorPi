...
import importlib
import os
from .exceptions import InterfaceTypMissingException, InterfaceNameAlreadyExistsException
...
class InterfaceHandler:
    ...
    def start(self):
        ...
        for root, sub_dirs, files in os.walk(interface_base_path):
            if len(sub_dirs) == 0:
                DOORPI.CONST.INTERFACES_AVAILABLE.append(root.replace(interface_base_path, "").replace(os.sep, ".")[1:])
        logger.info("founded %s installed interfaces" % len(DOORPI.CONST.INTERFACES_AVAILABLE))
        for interface_name in DOORPI.CONST.INTERFACES_AVAILABLE:
