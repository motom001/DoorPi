...
class ConfigObject():
    ...
    def load_config(configfile, search_for_defaults=True):
        ...
        if not configfile_name:
            logger.error('No valid configfile found - use parameter --configfile "filename"'
                         ' to specify one')
    ...
    def get_string(self, section, key, default='', log=True, password=False,
        ...
        if key.endswith('password') or password:
            if log: logger.trace("get_string for key %s in section %s (default: %s) returns %s",
                                 key, section, default, '*******')
        else:
            if log: logger.trace("get_string for key %s in section %s (default: %s) returns %s",
                                 key, section, default, value)
        return value

    def get_float(self, section, key, default=-1, log=True, store_if_not_exists=True):
        value = self.get(section, key, "", log=False, store_if_not_exists=store_if_not_exists)
        if value != '': value = float(value)
        else: value = default
        if log: logger.trace("get_float for key %s in section %s (default: %s) returns %s",
                             key, section, default, value)
        return value
    ...
    def get_integer(self, section, key, default=-1, log=True, store_if_not_exists=True):
        ...
        if log: logger.trace("get_integer for key %s in section %s (default: %s) returns %s",
                             key, section, default, value)
        return value

    def get_boolean(self, section, key, default=False, log=True, store_if_not_exists=True):
        value = self.get(section, key, default, log=False, store_if_not_exists=store_if_not_exists)
        value = value.lower() in ['true', 'yes', 'ja', '1']
        if log: logger.trace("get_boolean for key %s in section %s (default: %s) returns %s",
                             key, section, default, value)
        return value
    ...
    def get_list(self, section, key, default=[], separator=',', log=True,
                 store_if_not_exists=True):
        value = self.get(section, key, "", log=False, store_if_not_exists=store_if_not_exists)
        if value != '': value = value.split(separator)
        else: value = default
        if log: logger.trace("get_list for key %s in section %s (default: %s) returns %s",
                             key, section, default, value)
        return value
