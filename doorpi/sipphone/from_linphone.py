...
class LinPhone(SipphoneAbstractBaseClass):
    ...
    def __init__(self, whitelist=list(), *args, **kwargs):
        ...
        self.__Lib = lin.Core.new(
            self.callback.used_callbacks, config_path, factory_config_path)
        self.core.primary_contact = (
            '{} <sip:doorpi@127.0.0.1>').format(conf.get(SIPPHONE_SECTION, 'identity', 'DoorPi'))
    ...
    def start(self):
        ...
        self.core.sip_transports = lin.SipTransports(conf.get_int(
            SIPPHONE_SECTION, 'local_port', 5060), conf.get_int(SIPPHONE_SECTION, 'local_port', 5060), -1, -1)
        ...
        firewall_policy = conf.get(
            SIPPHONE_SECTION, 'FirewallPolicy', 'PolicyNoFirewall')
        ...
        self.core.in_call_timeout = conf.get_int(
            SIPPHONE_SECTION, 'max_call_time', 120)
        # If an incoming call isn’t answered for this timeout period, it is automatically declined.
        self.core.inc_timeout = conf.get_int(
            SIPPHONE_SECTION, 'call_timeout', 15)
        ...
        if len(self.core.sound_devices) == 0:
            ...
            self.core.capture_device = conf.get(
                SIPPHONE_SECTION, 'capture_device', self.core.capture_device)
            self.core.playback_device = conf.get(
                SIPPHONE_SECTION, 'playback_device', self.core.playback_device)
            self.core.mic_gain_db = conf.get_float(
                SIPPHONE_SECTION, 'mic_gain_db', 0)
            ...
            for sound_device in self.core.sound_devices:
                logger.debug('| %s | %s  | %s',
                             'X' if self.core.sound_device_can_capture(
                                 sound_device) else 'O',
                             'X' if self.core.sound_device_can_playback(
                                 sound_device) else 'O',
                             sound_device)
            logger.debug('------------------------------------')
        ...
        config_audio_codecs = conf.get_list(
            SIPPHONE_SECTION, 'audio_codecs', 'PCMA,PCMU')
        logger.debug('found %s supported audio codecs',
                     len(self.core.audio_codecs))
        logger.debug('------------------------------------')
        ...
        if (len(self.core.video_devices) == 0 or
           not conf.get_boolean(SIPPHONE_SECTION, 'video', False)):
            ...
            config_camera = conf.get(
                SIPPHONE_SECTION, 'video_device', self.core.video_devices[0])
            if config_camera not in self.core.video_devices:
                logger.warning(
                    'camera "%s" from config does not exist in possible video devices.', config_camera)
                logger.debug(
                    'switching to first possible video device "%s"', self.core.video_devices[0])
        ...
        config_video_codecs = conf.get_list(
            SIPPHONE_SECTION, 'video_codecs', 'VP8')
        logger.debug('found %s supported video codecs',
                     len(self.core.video_codecs))
        logger.debug('------------------------------------')
        ...
        if server and username and password:
            ...
            auth_info = self.core.create_auth_info(
                username, None, password, None, None, realm)
    ...
    def self_check(self, *args, **kwargs):
        ...
        if self.current_call.state < lin.CallState.Connected:
            if self.current_call_duration > self.core.inc_timeout - 0.5:
                logger.info('call timeout - hangup current call after %s seconds (max. %s)',
            ...
            if int(self.current_call_duration) > self.core.in_call_timeout - 0.5:
                logger.info('max call time reached - hangup current call after %s seconds (max. %s)',
            ...
            if self.core.invite_with_params(number, self.base_config) is None:
                if DoorPi().event_handler.db.get_event_log_entries_count('OnSipPhoneMakeCallFailed') > 5:
                    ...
                    DoorPi().event_handler('OnSipPhoneMakeCallFailed',
                                           __name__, {'number': number})
                return None
            DoorPi().event_handler('OnSipPhoneMakeCall',
                                   __name__, {'number': number})
        elif number in self.current_call.remote_address.as_string_uri_only():
            if self.current_call_duration <= 2:
                logger.debug('same call %s again while call is running since %s seconds',
                             self.core.current_call.remote_address.as_string_uri_only(),
                             self.current_call_duration)
            else:
                logger.debug("pressed twice with call duration > 1 second. Hangup current calls.")
                self.core.terminate_all_calls()
        else:
            logger.debug('only one call at the same time possible')
            self.core.terminate_all_calls()
            self.call(number)

        DoorPi().event_handler('AfterSipPhoneMakeCall',
                               __name__, {'number': number})
